...
// 2023.11.12 calcボタンとdispボタンを統合する→関数の中身をcalcに移動させるだけでできた。
// 過去の任意のデータ（41行）をiPhoneで表示させたいなら、linesに当たる部分を任意のリストに置き換えて実現できるはずlist:[String] = [,,]
...
class GraphAViewController: UIViewController {
...
// chartsのviewに関連するビューにauto layoutが適用できない理由を調べる。
// 軸の最大値を整数値にしようと色々試したが、そもそもy軸は0 ~ 0.9とかもあり得るわけで、整数にするべきではないことがわかった。
