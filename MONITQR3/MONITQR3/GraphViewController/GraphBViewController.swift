...
class GraphBViewController: UIViewController {
...
// データ前処理用メソッドcalc()を記述
extension GraphBViewController {
    @objc func calc() {
...
//                let day = Int(data[sepDay...sepDay2]) // 日データ
            ...
            // 空データ（あるいは41行でない文字列）を発見した場合、スキップしてループを続行
            } else {
                continue
            }
        }
        // x軸の最大値を動的に設定（データサイズ+10としている）
