...
class ProjectBViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        // UILabel
        textLabel.text = "This is the second project for monitoring."
    ...
    @objc func cameraButtonPressed() {
        // インスタンス化
        let cameraBViewController = CameraBViewController()
        // self.present(secondViewController, animated: true, completion: nil)
        navigationController?.pushViewController(cameraBViewController, animated: true)
    }
    // ファイルボタンが押された際の挙動を記述
    @objc func fileButtonPressed() {
        // インスタンス化
        let fileBViewController = FileBViewController()
        // self.present(thirdViewController, animated: true, completion: nil)
        navigationController?.pushViewController(fileBViewController, animated: true)

    }
    // グラフの表示
    @objc func graphButtonPressed() {
        let graphBViewController = GraphBViewController()
        // self.present(graphBViewController, animated: true, completion: nil)
        // no such fileと怒られてしまったので、navugationで遷移を実装してみる
        navigationController?.pushViewController(graphBViewController, animated: true)
    }
}
