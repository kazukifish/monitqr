...
class ProjectCViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        // UILabel
        textLabel.text = "This is the third project for monitoring."
    ...
    @objc func cameraButtonPressed() {
        // インスタンス化
        let cameraCViewController = CameraCViewController()
        // self.present(secondViewController, animated: true, completion: nil)
        navigationController?.pushViewController(cameraCViewController, animated: true)
    }
    // ファイルボタンが押された際の挙動を記述
    @objc func fileButtonPressed() {
        // インスタンス化
        let fileCViewController = FileCViewController()
        // self.present(thirdViewController, animated: true, completion: nil)
        navigationController?.pushViewController(fileCViewController, animated: true)

    }
    // グラフの表示
    @objc func graphButtonPressed() {
        let graphCViewController = GraphCViewController()
        // self.present(graphBViewController, animated: true, completion: nil)
        // no such fileと怒られてしまったので、navugationで遷移を実装してみる
        navigationController?.pushViewController(graphCViewController, animated: true)
    }
}
